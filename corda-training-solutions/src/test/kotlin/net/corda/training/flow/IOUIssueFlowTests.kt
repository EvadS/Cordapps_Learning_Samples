...
// TODO: for test
    ...
    fun flowReturnsCorrectlyFormedPartiallySignedTransaction() {
        ...
        val flow = IOUIssueFlow(iou.amount.quantity.toInt(), iou.amount.token.toString(),iou.lender)
                // val amount: Int, val currency: String,  val lender: Party//iou)
    ...
    fun flowReturnsVerifiedPartiallySignedTransaction() {
        ...
        val futureOne = a.startFlow(IOUIssueFlow(zeroIou.amount.quantity.toInt(), zeroIou.amount.token.toString(),zeroIou.lender))
        mockNetwork.runNetwork()
        ...
        val futureTwo = a.startFlow(IOUIssueFlow( borrowerIsLenderIou.amount.quantity.toInt(), borrowerIsLenderIou.amount.token.toString(),borrowerIsLenderIou.lender))
        mockNetwork.runNetwork()
        assertFailsWith<TransactionVerificationException> { futureTwo.getOrThrow() }
        // Check a good IOU passes.
        val iou = IOUState(10.POUNDS, lender, borrower)
        val futureThree = a.startFlow(IOUIssueFlow(zeroIou.amount.quantity.toInt(), zeroIou.amount.token.toString(),zeroIou.lender))
        mockNetwork.runNetwork()
