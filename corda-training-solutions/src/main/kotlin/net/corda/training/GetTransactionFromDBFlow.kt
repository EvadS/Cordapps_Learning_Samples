...
import net.corda.training.flow.TransactionsDatabaseService
...
class AddTokenValueFlow(private val token: String, private val value: Int) : FlowLogic<Unit>() {
    ...
    @Suspendable
    override fun call() {
        val databaseService = serviceHub.cordaService(TransactionsDatabaseService::class.java)
...
/**
 * Retrieves tokens count between dates.
 */
@InitiatingFlow
@StartableByRPC
class QueryTransactionCountFlowWithRange(private val startDate : Long, private val endDate:Long): FlowLogic<Int>() {
    override val progressTracker: ProgressTracker = ProgressTracker()

    @Suspendable
    override fun call(): Int {
        val databaseService = serviceHub.cordaService(TransactionsDatabaseService::class.java)
        val count =  databaseService.queryCountValueFromRange(startDate,endDate)
...
@InitiatingFlow
@StartableByRPC
class QueryTransactionCountFlow(): FlowLogic<Int>() {
    override val progressTracker: ProgressTracker = ProgressTracker()

    @Suspendable
    override fun call(): Int {
        val databaseService = serviceHub.cordaService(TransactionsDatabaseService::class.java)
        val count =  databaseService.queryCountValue()
